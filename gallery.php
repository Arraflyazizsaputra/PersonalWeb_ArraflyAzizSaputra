...
<head>
  ...
  .custom-scroll::-webkit-scrollbar-track {
    background: #1e3a8a; /* biru gelap */
    border-radius: 10px;
  }

  /* Scrollbar thumb (bagian geser) */
  .custom-scroll::-webkit-scrollbar-thumb {
    background: linear-gradient(to right, #facc15, #fde68a); /* gradasi kuning */
    border-radius: 10px;
    border: 2px solid #1e3a8a;
  }

  .custom-scroll::-webkit-scrollbar-thumb:hover {
    background: linear-gradient(to right, #facc15, #fcd34d); /* efek hover */
  }
</style>

</head>

<body class="bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 min-h-screen text-white font-inter">

  <!-- Header -->
  <header class="text-center py-6 shadow-lg">
    <h1 class="text-3xl md:text-3xl font-bold tracking-wide">📸 GALLERY | <span class="font-pacifico text-yellow-300">ARRAFLY AZIZ SAPUTRA</span></h1>
  </header>
   <!-- Navigation -->
...
<script>
  ...
  <main class="max-w-7xl mx-auto px-6 py-10">
    <h2 class="text-center text-2xl md:text-3xl font-bold mb-1">✨ Koleksi Terbaru Saya ✨</h2>
    <h3 class="text-center font-bold mb-8">Bisa di Klik dan di Downlod</h3>
   <div class="overflow-x-auto custom-scroll">
      <div class="flex space-x-6 w-max pb-4">
        ...
        while ($data = mysqli_fetch_array($query)) {
            echo "
            <div class='bg-white/20 backdrop-blur-md p-4 rounded-xl shadow-lg hover:scale-105 transition-all duration-300 border border-blue-200 w-64 flex-shrink-0 text-center'>
             <img 
  src='images/{$data['foto']}'
  onclick='openLightbox(this.src)' 
  class='w-48 h-48 object-cover mx-auto rounded-full border-4 border-white shadow-md hover:rotate-1 transition duration-300 cursor-pointer'
  alt='Gambar'>

              <h3 class='mt-4 text-lg font-semibold text-yellow-300 drop-shadow-sm'>".htmlspecialchars($data['judul'])."</h3>
            </div>
            ";
        }
        ?>
  ...
  <!-- Footer -->
  <footer class="bg-blue-800 text-center py-5 text-sm mt-10">
    &copy; <?php echo date('Y'); ?> | Dibuat oleh <span class="font-semibold text-yellow-300">ArraflyAzizSaputra</span>
...
<div id="lightbox" class="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center hidden z-50">
  <div class="relative">
    <img id="lightbox-img" src="" alt="Preview" class="max-w-[90vw] max-h-[80vh] rounded-xl shadow-lg border-4 border-white">
    <a id="download-btn" href="#" download class="absolute top-2 right-2 bg-yellow-400 text-blue-900 font-semibold px-4 py-1 rounded-xl hover:bg-yellow-300 shadow">
      ⬇️ Download
    </a>
    <button onclick="closeLightbox()" class="absolute top-2 left-2 text-white bg-red-500 hover:bg-red-600 px-3 py-1 rounded shadow">✖</button>
  </div>
</div>
