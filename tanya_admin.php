...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($nama) && !empty($pesan)) {
        $query = "INSERT INTO tbl_pesan (nama, pesan) VALUES ('$nama', '$pesan')";
        ...
        echo "<script>alert('Nama dan pesan harus diisi!');</script>";
    }
}
?>
...
<header class="bg-gradient-to-r from-blue-900 via-blue-700 to-cyan-400 text-white text-center py-8 px-4 shadow-lg">
  <div class="flex flex-col items-center space-y-2 md:flex-row md:space-y-0 md:space-x-4 md:justify-center animate-pulse">
    ...
    <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-3xl font-bold tracking-wide drop-shadow-lg">
      TANYA ADMIN | <span class="text-yellow-300">ARRAFLY AZIZ SAPUTRA</span>
    </h1>
...
<section class="max-w-3xl mx-auto mt-10 px-4">
  <div class="bg-gradient-to-br from-blue-900 via-blue-700 to-cyan-500 rounded-3xl shadow-2xl p-8 text-white relative overflow-hidden">
    
    <!-- Judul dan Deskripsi -->
    <h2 class="text-3xl font-extrabold mb-3 text-center drop-shadow-lg">📝 Tanya Admin</h2>
    <p class="text-center text-blue-100 mb-6 text-sm italic">Yuk, tanya apa saja ke admin! Kami siap menjawab dengan hati 💙</p>

    <!-- Form -->
    <form method="POST" action="tanya_admin.php" class="space-y-5">
      
      <!-- Input Nama -->
      <div class="relative">
        <label for="nama" class="block mb-1 font-semibold text-white">🧑 Nama Anda</label>
        <input type="text" name="nama" id="nama" placeholder="Masukkan nama kamu"
          class="w-full px-4 py-2 rounded-xl bg-white text-blue-900 shadow focus:outline-none focus:ring-4 focus:ring-blue-400 placeholder-gray-400">
      </div>

      <!-- Textarea Pesan -->
      <div class="relative">
        <label for="pesan" class="block mb-1 font-semibold text-white">💬 Pertanyaan</label>
        <textarea name="pesan" id="pesan" rows="4" placeholder="Apa yang ingin kamu tanyakan?"
          class="w-full px-4 py-2 rounded-xl bg-white text-blue-900 shadow focus:outline-none focus:ring-4 focus:ring-cyan-400 placeholder-gray-400 resize-none"></textarea>
      </div>

      <!-- Tombol Submit -->
      <div class="text-center">
        <button type="submit"
          class="bg-yellow-300 text-blue-900 px-6 py-2 rounded-full font-bold shadow-lg hover:bg-yellow-400 hover:scale-105 transition-all duration-300 ease-in-out">
          🚀 Kirim Sekarang
        </button>
      </div>
    </form>
    ...
    <!-- Ornamen Unik -->
    <div class="absolute -top-6 -right-6 text-7xl opacity-10 select-none">✉️</div>
    <div class="absolute -bottom-6 -left-6 text-7xl opacity-10 select-none">📬</div>
  </div>
</section>
