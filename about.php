...
<head>
  <meta charset="UTF-8">
  <title>About | ArraflyAzizSaputra</title>
  ...
  <style>
    ::selection { background-color: #93c5fd; color: white; }
  </style>
</head>

<body class="bg-gradient-to-br from-blue-900 via-blue-800 to-cyan-600 text-white font-bubble">

  <!-- Header -->
  <header class="text-center py-5">
    <h1 class="text-4xl font-bold tracking-wider drop-shadow-lg">TENTANG SAYA</h1>
    <p class="text-3xl mt-2 animate-bounce">😄</p>
  </header>
...
<script>
  ...
  <div class="flex flex-col md:flex-row bg-white bg-opacity-10 backdrop-blur-md rounded-3xl shadow-xl p-8 animate-fadeInUp space-y-8 md:space-y-0 md:space-x-10">

    <!-- Gambar Profil -->
    <div class="flex-shrink-0 text-center">
      <img src="img/gambar about.jpg" alt="Foto Profil" class="w-40 h-40 md:w-52 md:h-52 rounded-full border-4 border-yellow-300 shadow-lg hover:scale-105 transition duration-300 object-cover mx-auto">
      <p class="mt-4 text-yellow-300 font-bold text-lg">Arrafly Aziz Saputra</p>
    </div>

    <!-- Konten About -->
    <div class="flex-1">
      <h2 class="text-2xl font-bold mb-6 text-yellow-300">🌟 Kenalan Yuk!</h2>
      <div class="space-y-4 text-white text-lg leading-relaxed">
        <?php
        $sql = "SELECT * FROM tbl_about ORDER BY id_about DESC";
        $query = mysqli_query($db, $sql);
        while ($data = mysqli_fetch_array($query)) {
          echo "<div class='border-l-4 border-yellow-400 pl-4'>";
  ...
  <!-- Footer -->
  <footer class="bg-blue-900 text-center text-sm py-4 mt-20 shadow-inner">
    &copy; <?php echo date('Y'); ?> | Buatan <span class="font-bold text-yellow-300">ArraflyAzizSaputra</span>
