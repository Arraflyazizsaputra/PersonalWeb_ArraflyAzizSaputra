...
#artikel-container, #sidebar-container {
...
<header class="bg-gradient-to-r from-blue-900 via-blue-700 to-cyan-400 text-white text-center py-8 px-4 shadow-lg">
  <div class="flex flex-col items-center space-y-2 md:flex-row md:space-y-0 md:space-x-4 md:justify-center animate-pulse">
    ...
    <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-3xl font-bold tracking-wide drop-shadow-lg">
      PERSONAL WEB | <span class="text-yellow-300">ARRAFLY AZIZ SAPUTRA</span>
    </h1>
...
<section class="max-w-6xl mx-auto p-6 mt-6">
  <form method="GET" action="" class="flex flex-col md:flex-row gap-4 bg-gradient-to-r from-blue-900 to-blue-600 p-4 rounded-xl shadow-lg">
    <div class="relative w-full">
      <input type="text" name="cari" placeholder="🔍 Cari artikel..." value="<?php echo isset($_GET['cari']) ? htmlspecialchars($_GET['cari']) : '' ?>"
        class="w-full py-3 pl-10 pr-4 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-400">
    ...
    <button type="submit" class="bg-yellow-400 hover:bg-yellow-300 text-blue-900 font-bold px-6 py-3 rounded-lg transition duration-300 shadow">
      Cari
    </button>
  </form>
</section>
...
<!-- Main Content -->
<main class="max-w-6xl mx-auto p-6 mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 h-[150vh]">


  <!-- Artikel Utama -->
  <section class="md:col-span-2 overflow-y-auto h-full bg-white dark:bg-black p-6 rounded-2xl shadow-xl border border-blue-200 dark:border-gray-700" id="artikel-container">

    <h2 class="text-2xl font-extrabold text-blue-800 dark:text-blue-300 mb-4 border-b pb-2">📰 Artikel Terbaru</h2>
    <div class="space-y-6">
      <?php
      $filter = isset($_GET['cari']) ? mysqli_real_escape_string($db, $_GET['cari']) : '';
      $sql = "SELECT * FROM tbl_artikel WHERE nama_artikel LIKE '%$filter%' OR isi_artikel LIKE '%$filter%' ORDER BY id_artikel DESC";
      $query = mysqli_query($db, $sql);
                while ($data = mysqli_fetch_array($query)) {
                  $id_artikel = $data['id_artikel'];
                  echo "<div id='artikel_$id_artikel' class='border-b pb-4 hover:bg-blue-50 dark:hover:bg-gray-800 transition p-4 rounded'>";
                  echo "<h3 class='text-lg font-bold text-blue-700 dark:text-yellow-400'>" . htmlspecialchars($data['nama_artikel']) . "</h3>";
                  $isi = strip_tags($data['isi_artikel']);
                  $isi_pendek = substr($isi, 0, 200);
                  echo "<p class='text-gray-700 dark:text-gray-300'>" . htmlspecialchars($isi_pendek) . "...</p>";
                  echo "<a href='detail_artikel.php?id=" . urlencode($data['id_artikel']) . "' class='text-blue-500 hover:underline font-medium mt-2 inline-block'>Baca selengkapnya →</a>";
                  echo "</div>";
                }
  ...
  <!-- Sidebar -->
<aside class="overflow-y-auto h-full bg-gradient-to-br from-blue-900 via-blue-800 to-blue-600 p-6 rounded-2xl shadow-xl text-white" id="sidebar-container">

  <h2 class="text-xl font-extrabold mb-1  pb-2 flex items-center gap-2">
    📋 Daftar Artikel 
  </h2>
  <h3 class="text-x1 font-extrabold  pb-2 flex items-center gap-2">(Klik pada tiap judul untuk langsung merujuk artikel terkait) </h3>
  <h3 class="text-x1 font-extrabold mb-1 border-b-2 border-yellow-300 pb-2 flex items-center gap-2">Ada menu kalendernya juga lho! </h3>
  <ul class="space-y-4">
    <?php
   ...
   while ($data = mysqli_fetch_array($query)) {
    $id_artikel = $data['id_artikel'];
    $emoji = $emojiList[$i % count($emojiList)];
    echo "<li class='bg-white dark:bg-gray-900 text-gray-800 dark:text-white rounded-xl px-4 py-3 shadow hover:shadow-lg transition transform hover:scale-[1.02] cursor-pointer'>";
    echo "<a href='#artikel_$id_artikel' class='scroll-link'><span class='font-semibold'>$emoji " . htmlspecialchars($data['nama_artikel']) . "</span></a>";
    echo "</li>";
    $i++;
}
  ...
  <!-- Kalender -->
  <div class="mt-10 p-4 bg-white dark:bg-gray-900 text-gray-800 dark:text-white rounded-xl shadow-lg">
    <h3 class="text-lg font-bold mb-3 text-center text-blue-800 dark:text-yellow-400">📆 Kalender</h3>
  ...
  days.forEach(day => {
    ...
    el.className = 'font-bold text-blue-700 dark:text-yellow-400';
    el.textContent = day;
  ...
  for (let d = 1; d <= lastDate; d++) {
    const el = document.createElement('div');
    el.textContent = d;
    el.className = 'p-2 rounded hover:bg-yellow-300 hover:text-blue-900 transition duration-200';
    
    // Highlight hari ini
    if (d === date.getDate()) {
      el.classList.add('bg-yellow-400', 'text-blue-900', 'font-bold');
    }
  ...
  document.querySelectorAll('.scroll-link').forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      if (target && container) {
        ...
        // Scroll dalam container
        container.scrollTo({
          top: container.scrollTop + offset - (container.clientHeight / 2) + (target.offsetHeight / 2),
          behavior: 'smooth'
        });

        // Efek highlight sementara
        target.classList.add('ring-4', 'ring-yellow-400', 'bg-yellow-50', 'dark:bg-gray-800');
        setTimeout(() => {
          target.classList.remove('ring-4', 'ring-yellow-400', 'bg-yellow-50', 'dark:bg-gray-800');
        }, 2000);
      }
    });
  });
