...
<head>
    <meta charset="UTF-8">
    <title>Kelola Chat Pengunjung</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <header class="bg-blue-900 text-white text-center py-6 shadow">
        <h1 class="text-3xl font-bold">Kelola Chat Pengunjung</h1>
    </header>
    ...
    <div class="flex max-w-7xl mx-auto mt-8 px-4">
        ...
        <main class="w-3/4 bg-white rounded shadow p-6 ml-6">
            <form action="hapus_pesan.php" method="post" class="space-y-6">
                <div class="max-w-5xl mx-auto mt-8 p-4 bg-white rounded shadow">
                    <h2 class="text-xl font-bold mb-4">Daftar Pesan Pengunjung</h2>
                    <table class="w-full table-auto border-collapse border border-gray-300">
                        ...
                        <tbody>
                            <?php
                            $result = mysqli_query($db, "SELECT * FROM tbl_pesan ORDER BY tanggal DESC");
                            $no = 1;
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<tr>";
                                echo "<td class='border p-2 text-center'>" . $no++ . "</td>";
                                echo "<td class='border p-2'>" . htmlspecialchars($row['nama']) . "</td>";
                                echo "<td class='border p-2'>" . nl2br(htmlspecialchars($row['pesan'])) . "</td>";
                                echo "<td class='border p-2'>" . $row['tanggal'] . "</td>";
                                echo "<td class='border p-2 text-center'>
                                        <a href='hapus_pesan.php?id=" . $row['id_pesan'] . "' onclick='return confirm(\"Yakin ingin menghapus?\")' class='text-red-600 hover:underline'>Hapus</a>
                                      </td>";
                                echo "</tr>";
                            }
                            ?>
