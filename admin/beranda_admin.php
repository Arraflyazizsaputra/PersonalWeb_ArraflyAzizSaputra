...
<body class="bg-gradient-to-br from-blue-900 via-blue-700 to-blue-500 min-h-screen text-white font-sans">
    <!-- Header -->
    <header class="bg-blue-950 bg-opacity-90 text-white text-center py-6 shadow-lg">
        <h1 class="text-4xl font-extrabold tracking-wide uppercase">Halaman Administrator</h1>
    </header>

    <!-- Container -->
    <div class="flex flex-col lg:flex-row max-w-7xl mx-auto mt-10 px-4 gap-6">
        <!-- Sidebar -->
        <aside class="lg:w-1/4 w-full glass p-6 rounded-2xl shadow-xl">
            <h2 class="text-2xl font-bold text-center text-white mb-4">MENU</h2>
            <ul class="space-y-3 text-white text-lg">
                <li><a href="beranda_admin.php" class="block hover:text-yellow-300 transition">🏠 Beranda</a></li>
                <li><a href="data_artikel.php" class="block hover:text-yellow-300 transition">📝 Kelola Artikel</a></li>
                <li><a href="data_gallery.php" class="block hover:text-yellow-300 transition">🖼️ Kelola Gallery</a></li>
                <li><a href="about.php" class="block hover:text-yellow-300 transition">👤 About</a></li>
                <li><a href="kelola_pesan.php" class="block hover:text-yellow-300 transition">📬 Daftar Pesan</a></li>
                <li>
                    <a href="logout.php" onclick="return confirm('Apakah anda yakin ingin keluar?');"
                        class="block text-red-400 hover:text-red-600 font-semibold transition">🚪 Logout</a>
                </li>
            </ul>
        </aside>

        <!-- Main Content -->
        <main class="lg:w-3/4 w-full glass p-6 rounded-2xl shadow-xl">
            <div class="text-xl font-medium mb-4">
                Halo, <span class="font-bold text-yellow-300"><?= htmlspecialchars($username); ?></span>! Apa kabar? 😊
            </div>
            <p class="text-md text-white mb-6">Silakan gunakan menu di samping untuk mengelola data.</p>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Card Artikel -->
                <div class="glass p-6 rounded-xl shadow-lg border border-white/30 text-center hover:scale-105 transform transition">
                    <h3 class="text-2xl font-bold text-white mb-2">📝 Artikel</h3>
                    <p class="text-4xl font-extrabold text-yellow-200"><?= $jumlah_artikel; ?></p>
                </div>
                <!-- Card Gallery -->
                <div class="glass p-6 rounded-xl shadow-lg border border-white/30 text-center hover:scale-105 transform transition">
                    <h3 class="text-2xl font-bold text-white mb-2">🖼️ Gallery</h3>
                    <p class="text-4xl font-extrabold text-green-200"><?= $jumlah_gallery; ?></p>
                </div>
            </div>
        </main>
    </div>
    ...
    <!-- Footer -->
    <footer class="text-center text-white py-5 mt-20 text-sm">
        &copy; <?= date('Y'); ?> | Buatan ArraflyAzizSaputra
